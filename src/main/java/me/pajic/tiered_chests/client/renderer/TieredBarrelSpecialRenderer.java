...
public class TieredBarrelSpecialRenderer implements SpecialModelRenderer<Integer> {
    ...
    public @Nullable Integer extractArgument(ItemStack stack) {
        ...
        if (TieredChests.CONFIG.fancyBarrels.get()) state |= 2;
        return state;
    }
    ...
    public void submit(@Nullable Integer argument, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, int lightCoords, int overlayCoords, boolean hasFoil, int outlineColor) {
        ...
        if (tier != ChestTier.WOOD) {
            ...
            renderFace(submitNodeCollector, poseStack, RenderTypes.entityCutout(tierSide), lightCoords,
                    1f - offset, offset, -zO, offset, offset, -zO, offset, 1f - offset, -zO, 1f - offset, 1f - offset, -zO,
                    1f, 0f, 0f, 1f, 0, 0, -1); // North
            renderFace(submitNodeCollector, poseStack, RenderTypes.entityCutout(tierSide), lightCoords,
                    offset, offset, 1f + zO, 1f - offset, offset, 1f + zO, 1f - offset, 1f - offset, 1f + zO, offset, 1f - offset, 1f + zO,
                    0f, 1f, 1f, 0f, 0, 0, 1); // South
            renderFace(submitNodeCollector, poseStack, RenderTypes.entityCutout(tierSide), lightCoords,
                    -zO, offset, offset, -zO, offset, 1f - offset, -zO, 1f - offset, 1f - offset, -zO, 1f - offset, offset,
                    0f, 1f, 1f, 0f, -1, 0, 0); // West
            renderFace(submitNodeCollector, poseStack, RenderTypes.entityCutout(tierSide), lightCoords,
                    1f + zO, offset, 1f - offset, 1f + zO, offset, offset, 1f + zO, 1f - offset, offset, 1f + zO, 1f - offset, 1f - offset,
                    1f, 0f, 0f, 1f, 1, 0, 0); // East
            renderFace(submitNodeCollector, poseStack, RenderTypes.entityCutout(tierTop), lightCoords,
                    offset, 1f + zO, 1f - offset, 1f - offset, 1f + zO, 1f - offset, 1f - offset, 1f + zO, offset, offset, 1f + zO, offset,
                    0f, 1f, 1f, 0f, 0, 1, 0); // Top
            renderFace(submitNodeCollector, poseStack, RenderTypes.entityCutout(tierBottom), lightCoords,
                    offset, -zO, offset, 1f - offset, -zO, offset, 1f - offset, -zO, 1f - offset, offset, -zO, 1f - offset,
