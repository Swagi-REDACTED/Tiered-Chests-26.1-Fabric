 ...
 * extractArgument() can return the current config state. This value is appended to the
 * ItemStackRenderState model identity, busting the inventory render cache whenever the
...
public class TieredChestSpecialRenderer implements SpecialModelRenderer<Integer> {
    ...
    public TieredChestSpecialRenderer(SpecialModelRenderer.BakingContext context, ChestTier tier) {
        ...
        try { this.modelNoLock.root().getChild("lock").visible = false; } catch (Exception e) {}
        this.tier = tier;
    }

    /**
     * Return the config state as the argument. Minecraft appends this to the
     * ItemStackRenderState identity so the cache is busted whenever the value changes.
     */
    @Override
    public @Nullable Integer extractArgument(ItemStack stack) {
        // Encode both booleans into a single unique integer state
        int state = 0;
        if (TieredChests.CONFIG.texturePackOverride.get()) state |= 1;
        if (TieredChests.CONFIG.fancyChests.get()) state |= 2;
        return state;
    }
    ...
    public void submit(
        ...
        ChestModel tierModel = overrideOn ? modelNoLock : model;
        submitNodeCollector.submitModel(tierModel, openness, poseStack, lightCoords, overlayCoords, -1, tierSprite, this.sprites, outlineColor, null);
    }
