...
public class FabricClientEntrypoint implements ClientModInitializer {
    ...
    public static ParticleResult getTierParticleColor(ChestTier tier, boolean isFancy) {
        ...
        // Wood/Generic Tints
        if (isFancy) {
            return new ParticleResult(0xBBA685, false); // Subtle Fancy Wood
        } else {
    ...
    public static boolean shouldWash(Block block) {
        ...
        boolean isFancy = TieredChests.CONFIG.fancyChests.get();
    ...
    public void onInitializeClient() {
        try {
            ...
            ModBlockEntities.TIERED_BARRELS.values()
                    .forEach(type -> BlockEntityRenderers.register(type, me.pajic.tiered_chests.client.renderer.TieredBarrelRenderer::new));
            ...
            for (ChestTier tier : ChestTier.values()) {
                ...
                BlockColorRegistry.register(List.of(state -> {
                    boolean isFancy = TieredChests.CONFIG.fancyChests.get();
                    if (tier == ChestTier.WOOD && state.getBlock() instanceof me.pajic.tiered_chests.block.TieredBarrelBlock) {
                        isFancy = false;
                    }
