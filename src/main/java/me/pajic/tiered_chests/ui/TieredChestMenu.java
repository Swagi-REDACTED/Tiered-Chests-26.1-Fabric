...
public class TieredChestMenu extends AbstractContainerMenu {
    ...
    public TieredChestMenu(int containerId, Inventory inventory, ModNetworking.S2CTieredChestPayload payload) {
        this(containerId, inventory, new SimpleContainer(payload.tier().getSlotCount() * (payload.isDouble() ? 2 : 1)), payload.tier(), payload.pos());
    }
    ...
    public TieredChestMenu(int containerId, Inventory inventory, Container container, ChestTier tier, BlockPos pos) {
        ...
        this.rows = container.getContainerSize() / tier.getRowLength();
        this.columns = tier.getRowLength();
        ...
        int chestLeft = (this.imageWidth - chestWidth) / 2;
        int playerLeft = (this.imageWidth - PLAYER_INV_WIDTH) / 2;
